/*
Syrix Team Availability - v12.0 (Auto-Populate Lineups)
- FEATURE: Added 'Auto-Populate' button to Lineup Library.
- DATA: Included 'SAMPLE_LINEUPS' with standard tactical spots for Ascent, Bind, and Haven.
- FIX: Ensuring Lineup pins are interactive and editable.
*/
...
// --- SAMPLE DATA FOR AUTO-POPULATE (Common Knowledge Spots) ---
const SAMPLE_LINEUPS = {
    Ascent: [
        { x: 50, y: 50, title: 'Mid Link Smoke', agent: 'Omen', type: 'Smoke', description: 'Basic mid control smoke for Catwalk.', url: 'https://www.youtube.com/watch?v=dQw4w9WgXcQ' }, // Placeholder URL
        { x: 85, y: 25, title: 'A Site Recon', agent: 'Sova', type: 'Recon', description: 'Reveals back site and generator from A Main.', url: '' },
        { x: 15, y: 75, title: 'B Main Wall', agent: 'Sage', type: 'Barrier', description: 'Standard anti-rush wall for B Main.', url: '' },
        { x: 82, y: 20, title: 'A Generator Molly', agent: 'KAY/O', type: 'Molly', description: 'Clears the generator angle.', url: '' }
    ],
    Bind: [
        { x: 40, y: 40, title: 'Hookah Flash', agent: 'Skye', type: 'Flash', description: 'Pop flash for B push out of Hookah.', url: '' },
        { x: 80, y: 60, title: 'Showers Trip', agent: 'Cypher', type: 'Trap', description: 'Unbreakable trip for showers control.', url: '' },
        { x: 20, y: 50, title: 'B Site Smoke', agent: 'Brimstone', type: 'Smoke', description: 'Covers Elbow and CT.', url: '' }
    ],
    Haven: [
        { x: 50, y: 80, title: 'C Long Smoke', agent: 'Brimstone', type: 'Smoke', description: 'Deep smoke for C execution.', url: '' },
        { x: 50, y: 20, title: 'A Long Recon', agent: 'Sova', type: 'Recon', description: 'Early info dart for A lobby.', url: '' },
        { x: 10, y: 50, title: 'C Site Lockdown', agent: 'Killjoy', type: 'Ult', description: 'Standard retake ult location.', url: '' }
    ]
};
...
function LineupLibrary() {
    ...
    const populateDefaults = async () => {
        if (!SAMPLE_LINEUPS[selectedMap]) {
            addToast(`No sample data for ${selectedMap} yet.`, 'error');
            return;
        }

        // Check if default lineups already exist to prevent duplicates (simple check by title)
        const existingTitles = lineups.map(l => l.title);
        let addedCount = 0;

        for (const lineup of SAMPLE_LINEUPS[selectedMap]) {
            if (!existingTitles.includes(lineup.title)) {
                await addDoc(getTeamCollection('lineups'), {
                    ...lineup,
                    map: selectedMap,
                    addedBy: "System",
        ...
        if (addedCount > 0) {
            addToast(`Added ${addedCount} standard lineups.`);
        } else {
            addToast("Standard lineups already exist.");
        }
    };
        ...
        <div className="h-full flex flex-col gap-6">
            <div className="flex justify-between items-center">
                <div className="flex flex-col space-y-1">
                    ...
                    <div className="text-xs text-neutral-500">Click map to add pin, or use Auto-Populate.</div>
                </div>
                <ButtonSecondary onClick={populateDefaults} className="text-xs bg-red-900/20 border-red-900 text-red-400 hover:bg-red-900 hover:text-white">
                    ⚡ Auto-Populate {selectedMap}
                </ButtonSecondary>
            </div>
                    ...
                    {viewingLineup ? (
                        <div className="space-y-4 h-full flex flex-col">
                            ...
                            <div className="flex-1 bg-black/50 rounded-xl overflow-hidden border border-neutral-800 relative min-h-[200px]">
                                {viewingLineup.url && (viewingLineup.url.includes('youtube') || viewingLineup.url.includes('youtu.be')) ? (
                                    ...
                                    <div className="w-full h-full flex items-center justify-center text-neutral-500 italic">No media linked. Click 'Edit' to add a URL.</div>
                                )}
                            </div>
                            <p className="text-neutral-300 italic text-sm p-4 bg-black/30 rounded-lg border border-white/5">"{viewingLineup.description}"</p>

                            {/* Allow editing the URL if it's a placeholder or empty */}
                            <div className="flex gap-2">
                                <Input
                                    placeholder="Paste YouTube/Image URL here..."
                                    value={viewingLineup.url}
                                    onChange={async (e) => {
                                        // Optimistic update for UI
                                        const newUrl = e.target.value;
                                        setViewingLineup({ ...viewingLineup, url: newUrl });
                                        // Save to DB
                                        await updateDoc(getTeamDoc('lineups', viewingLineup.id), { url: newUrl });
                                    }}
                                />
                            </div>
        ...
        <div className="fixed inset-0 h-full w-full text-neutral-200 font-sans selection:bg-red-500/30 flex flex-col overflow-hidden">
            ...
            <main className="flex-1 overflow-y-auto p-6 scrollbar-thin scrollbar-thumb-red-900/50 scrollbar-track-black/20 relative z-10">
                <div className="max-w-[1920px] mx-auto min-h-screen flex flex-col">
                    {/* Added min-h-screen and flex-col to ensure content always has height */}
